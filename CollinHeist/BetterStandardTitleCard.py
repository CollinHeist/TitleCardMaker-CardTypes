...
class BetterStandardTitleCard(BaseCardType):
    ...
    __slots__ = (
        'souce_file', 'output_file', 'title', 'season_text', 'episode_text',
        'font_size', 'hide_season', 'vertical_shift', 'interline_spacing', 
    ...
    def __init__(self, *,
        ...
        super().__init__(blur if blur else (random() <= 0.1), grayscale)
    ...
    def _get_series_count_text_dimensions(self) -> dict:
        ...
        command = ' '.join([
            ...
            *self.__series_count_text_global_effects(),
            f'-font "{self.TITLE_FONT}"',    # Season text
            f'-gravity east',
            *self.__series_count_text_effects(),
            f'-annotate +1600+697.2 "{self.season_text} "',
            f'-font "{self.TITLE_FONT}"',   # Separator dot
            f'-gravity center',
            *self.__series_count_text_effects(),
            f'-annotate +0+689.5 "• "',
